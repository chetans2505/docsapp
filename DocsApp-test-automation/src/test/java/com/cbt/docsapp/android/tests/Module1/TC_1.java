package com.cbt.docsapp.android.tests.Module1;

import org.testng.annotations.Test;

import com.cbt.docsapp.util.BaseClass_Android;
import com.cbt.docsapp.util.ExecuteMapping_AndroidTestcases;

import org.testng.annotations.Parameters;

public class TC_1 extends BaseClass_Android{
	
	
	@Test(description = "Add description here")
	@Parameters({"Device Name"})
	public void TC_1(String DeviceName) throws Exception
	{
		// Write code for adding reports or logs
		System.out.println("In test");
		System.out.println("Test case parameters " + DeviceName);
	
		ExecuteMapping_AndroidTestcases eq = new ExecuteMapping_AndroidTestcases(aDriver, "android", "Module1", "TC_1");
		eq.executeAction();
	}
	
	
}
